...
//to the max range we can reach by taking on of the 1..A[i]. step++. but we don't know which we take(and we don't care).
//the in the new range (after previousRange, maxPlace). we do the same search for each of them.
//which one shall you step on and go? extend the range again by checking the max place we can reach out of these pos in the range
